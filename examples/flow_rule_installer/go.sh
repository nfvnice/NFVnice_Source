...
if [ -z $print ]
then
        sudo ./build/flow_rule_installer -l $cpu -n 3 --proc-type=secondary -- -r $service -- -d $dst
else
        sudo ./build/flow_rule_installer -l $cpu -n 3 --proc-type=secondary -- -r $service -- -d $dst  -s $sclen -p $print
fi
