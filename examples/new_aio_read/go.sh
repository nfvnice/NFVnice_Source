...
if [ -z $print ]
then
        sudo ./build/aio_aclread -l $cpu -n 3 --proc-type=secondary -- -r $service -- -d $dst
else
        sudo ./build/aio_aclread -l $cpu -n 3 --proc-type=secondary -- -r $service -- -d $dst  -s $sclen -p $print
fi
