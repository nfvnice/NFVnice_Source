...
if [ -z $print ]
then
        sudo ./build/performance_monitor -l $cpu -n 3 --proc-type=secondary -- -r $service -- -d $dst
else
        sudo ./build/performance_monitor -l $cpu -n 3 --proc-type=secondary -- -r $service -- -d $dst -p $print
fi
