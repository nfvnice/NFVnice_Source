...
if [ -z $print ]
then
        sudo ./build/pkt_logger -l $cpu -n 3 --proc-type=secondary -- -r $service -- -d $dst
else
        sudo ./build/pkt_logger -l $cpu -n 3 --proc-type=secondary -- -r $service -- -d $dst  -s $sclen -p $print
fi
