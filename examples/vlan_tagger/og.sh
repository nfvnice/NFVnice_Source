...
if [ -z $print ]
then
        sudo ./build/vlantagger -l $cpu -n 3 --proc-type=secondary -- -r $service -n $inst_id -- -d $dst
else
        sudo ./build/vlantagger -l $cpu -n 3 --proc-type=secondary -- -r $service -n $inst_id -- -d $dst -p $print
fi
